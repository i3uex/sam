...
def process_image_slice(sam_predictor: SamPredictor,
    ...
    if debug.enabled:
        if image_slice.labels.size > 1:
            ...
            debug_file_path = debug.get_file_path(f'prompt_y_{point_coords[-1][1]}', '.yml')
            ...
            debug_file_path = debug.get_file_path(f'prediction_y_{point_coords[-1][1]}', '.png')
...
def process_image(sam_predictor: SamPredictor,
    ...
    items = image.shape[-1]
    y_range = range(1083, 4016)
    y_range_items = 4016 - 1083
    progress_bar = tqdm(desc='Processing CT image slices', total=y_range_items)

    results = []
    for slice_number in range(items):
        for y in y_range:
            center = np.array([2675, y])
